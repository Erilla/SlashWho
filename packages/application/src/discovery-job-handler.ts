...
export function createDiscoveryJobHandler(options: DiscoveryJobHandlerOptions) {
  ...
  return {
    async execute(
      ...
      return attributeThrottlesTo(scope, { runId }, async () => {
          ...
          if (outcome.kind === "snapshot") {
            ...
            if (fingerprint && blizzardGateway) {
              ...
              if (admission.kind === "not_due") {
                const live =
                  await repositories.fingerprintSweeps.getResumeState(
                    run.rootKey
                  );
              ...
              if (admission.kind === "admitted") {
                ...
                try {
                  ...
                  if (sweep.kind === "failure") {
                    ...
                    const newlyAdmittedFingerprintMatches =
                      deduplicateCharacters([...sweep.characters]).filter(
                        (character) =>
