...
| Blizzard Cutting Edge completions                                  | Web-process memory, per character       | 15 minutes after success; 400 entries and at most 400 pending loads                                        |
| Raider.IO guild boss rankings                                      | Web-process memory, per raid/boss query | 15 minutes after success; 1,000 entries and at most 1,000 pending loads                                    |
...
cache; only WCL scan reservations coordinate across them.
...
keys gives 400. Because the caches are process-local, each added `web` replica
holds its own copy, and the multiple is the headroom that covers it.
...
snapshot membership changes, least-recently-used memory eviction, shared token refresh,
